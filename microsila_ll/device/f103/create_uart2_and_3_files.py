...
def CreateUartFiles():
  ...
  # create .h files
  with open('./stm32f103_uart1.h', encoding='utf-8') as f:
    contents = f.read()
    uart2_contents = deepcopy(contents);
    uart2_contents = replace_uart(2, uart2_contents)
    with open('./stm32f103_uart2.h', 'w') as out:
      out.write(uart2_contents)
    uart3_contents = deepcopy(contents);
    uart3_contents = replace_uart(3, uart3_contents)
    with open('./stm32f103_uart3.h', 'w') as out:
      out.write(uart3_contents)

  # create .c files
  with open('./stm32f103_uart1.c', encoding='utf-8') as f:
    contents = f.read()
    uart2_contents = deepcopy(contents);
    uart2_contents = replace_uart(2, uart2_contents)
    with open('./stm32f103_uart2.c', 'w') as out:
      out.write(uart2_contents)
    uart3_contents = deepcopy(contents);
    uart3_contents = replace_uart(3, uart3_contents)
    with open('./stm32f103_uart3.c', 'w') as out:
      out.write(uart3_contents)
  print("Done.")
